    ...
    try:
        ...
        for obj in theobjects['Contents']:
            if obj['Key'].endswith('.csv'):
                ...
                print("Connection established with s3...")
        ...
        with open(os.path.join(main_webm.output_dir, 'errors.txt'), 'a', newline='') as f:
            ...
            main_webm.status = "ERROR: [S3 Sink Cols] - Something Went Wrong !!"


def dataframe(s3_uri, specific_cols):
    # s3://ideakafkatest/QA_UI_testing/ftp/3/sftp/sftpuser1/demo-data/facts/INVENTORIES_INDEX.csv
    try:
        ...
        s3_file_path = s3_uri.split(bucket_name)[1][1:]
        # QA_UI_testing/ftp/3/sftp/sftpuser1/demo-data/facts/INVENTORIES_INDEX.csv
        # print("--------",s3_file_path)
        s3_str = f's3://{config.aws_access_key_id}:{config.aws_secret_access_key}@{bucket_name}/{s3_file_path}'
        print("S3 Sink file path: ", f"s3://{bucket_name}/{s3_file_path}")
        ...
        with open(os.path.join(main_webm.output_dir, 'errors.txt'), 'a', newline='') as f:
            f.write(f"\n\n--- Exception Sink DF {datetime.now().replace(microsecond=0)}---\n{e}")
            main_webm.status = "ERROR: [S3 Sink DF] - Something Went Wrong !!"
        # logging.error("Exception: %s",e).


def concat_df(s3_uri, df_list, ty, specific_cols):
    try:
        print("S3 Sink Concatenating Dataframes")
        if specific_cols[0] in ["*", ""]:
            details(s3_uri)
        ...
        print(e)
        with open(os.path.join(main_webm.output_dir, 'errors.txt'), 'a', newline='') as f:
            f.write(f"\n\n--- Exception S3 Sink DF concat {datetime.now().replace(microsecond=0)}---\n{e}")
            main_webm.status = "ERROR: [S3 Sink concat] Something Went Wrong !!"
        # logging.error("Exception: %s",e)
...
def details(s3_uri):
    try:
        ...
        theobjects = s3_client.list_objects_v2(Bucket=bucket_name, Prefix=s3_file_path)
        for obj in theobjects['Contents']:
            if obj['Key'].endswith('.csv') and run:
                # print(obj['Key'])
                # Get Columns name and Columns count from S3 object
                resp = s3_client.select_object_content(
                    Bucket=bucket_name,
                ...
                resp = s3_client.select_object_content(
                    ...
                    # Comment this line while reading Parquet file
                    # InputSerialization={'CSV': {"FileHeaderInfo": "None"}, 'CompressionType': 'NONE'},
                    OutputSerialization={'CSV': {}},
                )
                for event in resp['Payload']:
                ...
                if run:  # Get Columns name and Columns count from S3 object
                    ...
                    for event in resp['Payload']:
                        if 'Records' in event:
                            ...
                            print(list(json.loads(cols).keys()))
                            columns = list(json.loads(cols).keys())
                            print("------cols", json.loads(cols).keys())
                            # print("-----------------------------------------------------------------------\n")
                            print("S3 Sink file Columns : ", tuple(columns))
                            print("S3 Sink file Columns count: ", len(columns))
        ...
        print(e)
        with open(os.path.join(main_webm.output_dir, 'errors.txt'), 'a', newline='') as f:
            f.write(f"\n\n--- Exception S3 Sink detail {datetime.now().replace(microsecond=0)}---\n{e}")
            main_webm.status = "[S3 Sink Details] Something Went Wrong !!"
